...
void printOnTotalValueFile(int value, Setup &setup)
{
    if (value == -1)
        return;
    outFile << "H(";
    for (int i = 0; i < int(setup.numFile + setup.numUser + setup.commonInformations + setup.demands.size()); i++)
    {
        long long define = 1ULL << i;
        if (value & define)
        {
            if (i < setup.numFile)
                outFile << "W" << i ;
            else if (i < setup.numFile + setup.numUser)
                outFile << "Z" << i - setup.numFile ;
            else if (i < setup.numFile + setup.numUser + setup.commonInformations)
                outFile << "C" << i - setup.numFile - setup.numUser ;
            else
            {
                outFile << "X";
                for (auto &d : setup.demands[i - setup.numFile - setup.numUser - setup.commonInformations])
                {
                    outFile << d ;
                }
            }
...
void printOnTotalValue(int value, Setup &setup)
{
    if (value == -1)
        return;
    cout << "H(";
    for (int i = 0; i < int(setup.numFile + setup.numUser + setup.commonInformations + setup.demands.size()); i++)
    {
        ...
        if (value & define)
        {
            if (i < setup.numFile)
                ...
                cout << "Z" << i - setup.numFile;
            else if (i < setup.numFile + setup.numUser + setup.commonInformations)
                cout << "C" << i - setup.numFile - setup.numUser;
            else
            {
                cout << "X";
                for (auto &d : setup.demands[i - setup.numFile - setup.numUser - setup.commonInformations])
...
void printValue(int value, Setup &setup)
{
    ...
    {
        outFile << "Z" << value - setup.numFile << " ";
    }
    else if (value < setup.numUser + setup.numFile + setup.commonInformations)
    {
        outFile << "C" << value - setup.numFile - setup.numUser << " ";
    }
    else
    {
...
void createTablePermutationsFull(Setup &setup, vector<vector<int>> &filePerms,
                                 vector<vector<int>> &userPerms, unordered_set<vector<int>, VectorHash> &allPermutations,
                                 int numSingleVar, unordered_set<vector<int>, VectorHash> &allPermutationsUserFile)
{
        ...
        for (auto &filePerm : filePerms)
        {
            for (auto &userPerm : userPerms)
            {
                ...
                if (totalNotFound == setup.demands.size())
                {
                    ...
                    // Add the first numUser + numFile int allPermtationsUserFile
                    allPermutationsUserFile.insert(vector<int>(P.begin(), P.begin() + setup.numUser + setup.numFile));
                    continue;
                }
        ...
        for (int j = 0; j <= setup.numUser; j++)
        {
            ...
            for (auto filevec : subsetsUsers)
            {
                ...
                for (auto cachevec : subsetsFiles)
                {
                    ...
                    // Add constraints only if they don't have any other symmetry
                    if (setup.equalVariables[jdx].main_symmetry == -1)
                    {
                        int toAddFirst = setup.equalVariables[jdx].final_position;
                        model.addConstr(bitmapTable[toAddFirst] == r, "symmetri_to_" + bitmapTable[toAddFirst].get(GRB_StringAttr_VarName));
                    }
                }
                subsetsFiles.clear();
            }
                                                     ...
                                                     GRBVar *bitmapTable)
{
    ...
    for (auto &subset : subsetsDemand)
    {
        ...
        for (; it != allPermutations.end(); ++it)
        {
            ...
            if ( setup.equalVariables[sumSubset].main_symmetry == -1 || setup.equalVariables[sumPermutation].main_symmetry == -1)
            {
                int firstToAdd = setup.equalVariables[sumSubset].main_symmetry == -1 ? setup.equalVariables[sumSubset].final_position : setup.equalVariables[setup.equalVariables[sumSubset].main_symmetry].final_position;
                int secondToAdd = setup.equalVariables[sumPermutation].main_symmetry == -1 ? setup.equalVariables[sumPermutation].final_position : setup.equalVariables[setup.equalVariables[sumPermutation].main_symmetry].final_position;

                model.addConstr(bitmapTable[firstToAdd] == bitmapTable[secondToAdd], "Equality between " + bitmapTable[firstToAdd].get(GRB_StringAttr_VarName) + " and " + bitmapTable[secondToAdd].get(GRB_StringAttr_VarName));
            }
            if (Debug > 1)
            {
                outFile << "H (";
                                     ...
                                     GRBVar *bitmapTable, int totalEquations)
{
    ...
    if( setup.equalVariables[entropyA + entropyC].main_symmetry == -1 || setup.equalVariables[entropyB + entropyC].main_symmetry == -1 || setup.equalVariables[entropyA + entropyB + entropyC].main_symmetry == -1 || setup.equalVariables[entropyC].main_symmetry == -1)
    {
    int realEntropyAC = setup.equalVariables[entropyA + entropyC].main_symmetry == -1 ? setup.equalVariables[entropyA + entropyC].final_position : setup.equalVariables[setup.equalVariables[entropyA + entropyC].main_symmetry].final_position;
    int realEntropyBC = setup.equalVariables[entropyB + entropyC].main_symmetry == -1 ? setup.equalVariables[entropyB + entropyC].final_position : setup.equalVariables[setup.equalVariables[entropyB + entropyC].main_symmetry].final_position;
    int realEntropyABC = setup.equalVariables[entropyA + entropyB + entropyC].main_symmetry == -1 ? setup.equalVariables[entropyA + entropyB + entropyC].final_position : setup.equalVariables[setup.equalVariables[entropyA + entropyB + entropyC].main_symmetry].final_position;
    int realEntropyC = setup.equalVariables[entropyC].main_symmetry == -1 ? setup.equalVariables[entropyC].final_position : setup.equalVariables[setup.equalVariables[entropyC].main_symmetry].final_position;
    // #pragma omp critical
        if (setup.maxValueSubsets[entropyC] == 0)
        {
            ...
            {
            model.addConstr(bitmapTable[realEntropyAC] + bitmapTable[realEntropyBC] - bitmapTable[realEntropyABC] - bitmapTable[realEntropyC] >= 0, "H(" + to_string(realEntropyAC) + ") + H(" + to_string(realEntropyBC) + ") - H(" + to_string(realEntropyABC) + ") - H(" + to_string(realEntropyC) + ") >= 0");
            }
            ...
            if (Debug > 1)
            {
                ...
                {
                    printValue(innerSubset[i], setup);
...
void addFileIndepenceConstraints(Setup &setup, GRBModel &model, GRBVar *bitmapTable)
{
    ...
    for (auto &subset : subsets)
    {
        ...
        if( setup.equalVariables[sumSubset].main_symmetry == -1)
        {
            int firstToAdd = setup.equalVariables[sumSubset].final_position;
            model.addConstr(bitmapTable[firstToAdd] == size, bitmapTable[firstToAdd].get(GRB_StringAttr_VarName) + " = " + to_string(size));
            totalEquations++;
            if (Debug > 1)
            {
                outFile << "H (";
                                   ...
                                   vector<vector<int>> &subsets, vector<int> &allVariables)
{
    ...
    for (auto &subset : subsets)
    {
        ...
        for (size_t i = 0; i < allVariables.size(); i++)
        {
            ...
            if (!setup.fullVariables)
            {
                cout << "Adding monotonicity constraint for subset: ";
                for (size_t j = 0; j < subset.size(); j++)
                {
                    printValue(subset[j], setup);
                    if (j != subset.size() - 1)
                        outFile << ",";
                }
                if (allVariables[i] < setup.numUser + setup.numFile + setup.commonInformations)
                {
                    if( setup.equalVariables[sumSubset].main_symmetry == -1 || setup.equalVariables[sumSubset + (1ULL << allVariables[i])].main_symmetry == -1)
                    {
                    int firstToAdd = setup.equalVariables[sumSubset + (1ULL << allVariables[i])].main_symmetry == -1 ? setup.equalVariables[sumSubset + (1ULL << allVariables[i])].final_position : setup.equalVariables[setup.equalVariables[sumSubset + (1ULL << allVariables[i])].main_symmetry].final_position;
                    int secondToAdd = setup.equalVariables[sumSubset].main_symmetry == -1 ? setup.equalVariables[sumSubset].final_position : setup.equalVariables[setup.equalVariables[sumSubset].main_symmetry].final_position;
                    model.addConstr(bitmapTable[firstToAdd] - bitmapTable[secondToAdd] >= 0, "H(" + bitmapTable[firstToAdd].get(GRB_StringAttr_VarName) + ") - H(" + bitmapTable[secondToAdd].get(GRB_StringAttr_VarName) + ") >= 0");
                    }
                }
                else
                {
                    if( setup.equalVariables[sumSubset].main_symmetry == -1 || setup.equalVariables[sumSubset + ((allVariables[i] - setup.numUser - setup.numFile - setup.commonInformations + 1) * (1ULL << (setup.numUser + setup.numFile + setup.commonInformations)))].main_symmetry == -1)
                    {
                    
                        int firstToAdd = setup.equalVariables[sumSubset + ((allVariables[i] - setup.numUser - setup.numFile - setup.commonInformations + 1) * (1ULL << (setup.numUser + setup.numFile + setup.commonInformations)))].main_symmetry == -1 ? 
                            setup.equalVariables[sumSubset + ((allVariables[i] - setup.numUser - setup.numFile - setup.commonInformations + 1) * (1ULL << (setup.numUser + setup.numFile + setup.commonInformations)))].final_position : 
                            setup.equalVariables[setup.equalVariables[sumSubset + ((allVariables[i] - setup.numUser - setup.numFile - setup.commonInformations + 1) * (1ULL << (setup.numUser + setup.numFile + setup.commonInformations)))].main_symmetry].final_position;
                        int secondToAdd = setup.equalVariables[sumSubset].main_symmetry == -1 ? setup.equalVariables[sumSubset].final_position : setup.equalVariables[setup.equalVariables[sumSubset].main_symmetry].final_position;
                        model.addConstr(bitmapTable[firstToAdd] - bitmapTable[secondToAdd] >= 0, "H(" + bitmapTable[firstToAdd].get(GRB_StringAttr_VarName) + ") - H(" + bitmapTable[secondToAdd].get(GRB_StringAttr_VarName) + ") >= 0");
                    }
                }
            ...
            else
            {
                if( setup.equalVariables[sumSubset].main_symmetry == -1 || setup.equalVariables[sumSubset + (1ULL << allVariables[i])].main_symmetry == -1)
                {
                    ...
                    if (Debug > 0)
                    {
                        ...
                        outFile << ",";
                        printValue(allVariables[i], setup);
                        outFile << ") - H (";
                        for (size_t j = 0; j < subset.size(); j++)
                        {
                            printValue(subset[j], setup);
                           ...
                           GRBVar &r)
{
    ...
    for (size_t i = 0; i < setup.demands.size(); i++)
    {
        if (!setup.fullVariables)
        {
            ...
            if( setup.equalVariables[demandVal].main_symmetry == -1)
            {
                int firstToAdd = setup.equalVariables[demandVal].final_position;
                model.addConstr(bitmapTable[firstToAdd] <= r, "MaximumRate_demand" + to_string(i));
                // TODO add Gurobi constraints
                if (Debug > 0)
                {
...
void addCacheDemandDependenceConstraints(Setup &setup, GRBModel &model, GRBVar *bitmapTable)
{
    ...
    for (int i = 0; i < setup.numUser; i++)
    {
        ...
        if( setup.equalVariables[userVal + allWvalues].main_symmetry == -1 || setup.equalVariables[allWvalues].main_symmetry == -1)
        {
            int firstToAdd = setup.equalVariables[userVal + allWvalues].main_symmetry == -1 ? setup.equalVariables[userVal + allWvalues].final_position : setup.equalVariables[setup.equalVariables[userVal + allWvalues].main_symmetry].final_position;
            int secondToAdd = setup.equalVariables[allWvalues].main_symmetry == -1 ? setup.equalVariables[allWvalues].final_position : setup.equalVariables[setup.equalVariables[allWvalues].main_symmetry].final_position;
            model.addConstr(bitmapTable[firstToAdd] - bitmapTable[secondToAdd] == 0,
    ...
    for (size_t i = 0; i < setup.demands.size(); i++)
    {
        ...
        if( setup.equalVariables[demandVal + allWvalues].main_symmetry == -1 || setup.equalVariables[allWvalues].main_symmetry == -1)
        {
            int firstToAdd = setup.equalVariables[demandVal + allWvalues].main_symmetry == -1 ? setup.equalVariables[demandVal + allWvalues].final_position : setup.equalVariables[setup.equalVariables[demandVal + allWvalues].main_symmetry].final_position;
            int secondToAdd = setup.equalVariables[allWvalues].main_symmetry == -1 ? setup.equalVariables[allWvalues].final_position : setup.equalVariables[setup.equalVariables[allWvalues].main_symmetry].final_position;
            model.addConstr(bitmapTable[firstToAdd] - bitmapTable[secondToAdd] == 0,
                            "H(" + bitmapTable[firstToAdd].get(GRB_StringAttr_VarName) + "| all W) = 0");
            // TODO add Gurobi constraints
            if (Debug > 0)
            {
                outFile << "H(X";
...
void addFileReconstructionConstraints(Setup &setup, GRBModel &model, GRBVar *bitmapTable)
{
    ...
    for (size_t i = 0; i < setup.demands.size(); i++)
    {
        ...
        for (int j = 0; j < setup.numUser; j++)
        {
            ...
            if( setup.equalVariables[fileVal + userVal + demandVal].main_symmetry == -1 || setup.equalVariables[userVal + demandVal].main_symmetry == -1)
            {
                int firstToAdd = setup.equalVariables[fileVal + userVal + demandVal].main_symmetry == -1 ? setup.equalVariables[fileVal + userVal + demandVal].final_position : setup.equalVariables[setup.equalVariables[fileVal + userVal + demandVal].main_symmetry].final_position;
                int secondToAdd = setup.equalVariables[userVal + demandVal].main_symmetry == -1 ? setup.equalVariables[userVal + demandVal].final_position : setup.equalVariables[setup.equalVariables[userVal + demandVal].main_symmetry].final_position;
                int dVal = setup.equalVariables[demandVal].main_symmetry == -1 ? setup.equalVariables[demandVal].final_position : setup.equalVariables[setup.equalVariables[demandVal].main_symmetry].final_position;
                model.addConstr(bitmapTable[firstToAdd] - bitmapTable[secondToAdd] == 0,
                                "H(W" + to_string(demand[j]) + "| Z" + to_string(j) + "," + bitmapTable[dVal].get(GRB_StringAttr_VarName) + ") = 0");
                // TODO add Gurobi constraints
                if (Debug > 0)
                {
...
void addKConstraints(Setup &setup, GRBModel &model, GRBVar *bitmapTable, vector<GRBConstr> &kConstraints)
{
    ...
    for (auto &group : setup.commonInformationsStrings)
    {
        ...
        // Changed indices to final positions
        sumBefore = setup.equalVariables[sumBefore].main_symmetry == -1 ? setup.equalVariables[sumBefore].final_position : setup.equalVariables[setup.equalVariables[sumBefore].main_symmetry].final_position;
        sumAfter = setup.equalVariables[sumAfter].main_symmetry == -1 ? setup.equalVariables[sumAfter].final_position : setup.equalVariables[setup.equalVariables[sumAfter].main_symmetry].final_position;
        kValue = setup.equalVariables[kValue].main_symmetry == -1 ? setup.equalVariables[kValue].final_position : setup.equalVariables[setup.equalVariables[kValue].main_symmetry].final_position;
        ...
        if (Debug > 0)
        {
            ...
            outFile << "H(" << kValue + sumBefore << ") - H(" << sumBefore << ") = 0" << endl;
            outFile << "H(" << kValue + sumAfter << ") - H(" << sumAfter << ") = 0" << endl;
        }
    }
}
...
void checkAllMValues(Setup &setup, GRBModel &model, GRBVar *bitmapTable)
{
    auto solveTradeoff = [&](double cacheSize)
    {
        ...
        for (int i = 0; i < setup.numUser; i++)
        {
            int position = 1 << (i + setup.numFile);
            if(setup.equalVariables[position].main_symmetry == -1 || setup.equalVariables[position].final_position == -1)
            {
                int firstToAdd = setup.equalVariables[position].main_symmetry == -1 ? setup.equalVariables[position].final_position : setup.equalVariables[setup.equalVariables[position].main_symmetry].final_position;
                auto constr = model.addConstr(bitmapTable[firstToAdd] <= cacheSize, "CacheSize_Z" + to_string(i) + "_M" + to_string(cacheSize));
                              ...
                              GRBModel &model, GRBVar *bitmapTable)
{
    if (!setup.fullVariables)
    {
        ...
        {
            int firstToAdd = setup.equalVariables[demandVal].final_position;
            model.addConstr(bitmapTable[firstToAdd] == setup.numFile, "H(" + bitmapTable[firstToAdd].get(GRB_StringAttr_VarName) + ") = " + to_string(setup.numFile));
        }
    }
    else
    {
...
void defineMaxDimensionDemands(Setup &setup, GRBModel &model, GRBVar *bitmapTable)
{
    ...
    for (size_t i = 0; i < subsets.size(); i++)
    {
        ...
        if (numContainedFiles == setup.numFile)
        {
            ...
            if (setup.equalVariables[value].main_symmetry == -1)
            {
                int firstToAdd =  setup.equalVariables[value].final_position;

                model.addConstr(bitmapTable[firstToAdd] == setup.numFile, "H(" + bitmapTable[firstToAdd].get(GRB_StringAttr::GRB_StringAttr_VarName) + ") = " + to_string(setup.numFile));
            }
        }
...
void renameVariables(GRBVar *bitmapTable, Setup &setup, int numDemands, int maxVar, GRBModel &model, bool symDelete)
{
    if (symDelete)
    {
        for (int i = 0; i < setup.equalVariables.size(); i++)
        {
            if (setup.equalVariables[i].main_symmetry != -1)
                continue;
            ...
            bitmapTable[setup.equalVariables[i].final_position].set(GRB_StringAttr::GRB_StringAttr_VarName, varName);
        }
    }
    else
    {
...
void addModelConstraints(Setup &setup, GRBModel &model, GRBVar *bitmapTable, chrono::high_resolution_clock::time_point &startChrono,
                         int maxVar, int numSingleVar, int SymVar)
{
                        ...
                        vector<pair<double, string>> &tradeoffs, string &timestamp)
{
    ...
    for (int i = 0; i < setup.numUser; ++i)
    {
        ...
        int position = 1 << (i + setup.numFile);
        int firstToAdd = setup.equalVariables[position].main_symmetry == -1 ? setup.equalVariables[position].final_position : setup.equalVariables[setup.equalVariables[position].main_symmetry].final_position;
        auto constr = model.addConstr(bitmapTable[firstToAdd] <= setup.M, "PersistentCacheUser" + to_string(i));
...
int findEqualVariablesUsingSymmetry(Setup &setup, int numSingleVar)
{
    int final_NumVar = 0;
    // Implement symmetry detection and variable reduction logic here
    // Update final_NumVar accordingly
    auto filePerms = vector<vector<int>>();
    auto userPerms = vector<vector<int>>();
    createUserFilePermutations(setup, userPerms, filePerms);
        ...
        for (int j = 0; j <= setup.numUser; j++)
        {
            ...
            for (auto filevec : subsetsUsers)
            {
                ...
                for (auto cachevec : subsetsFiles)
                {
                    ...
                    cachevecAll.push_back(jdx);
                }
                subsetsFiles.clear();
                for (auto val : cachevecAll)
                {
                    if (val != jdxBest && (setup.equalVariables[val].main_symmetry == -1 || setup.equalVariables[val].main_symmetry > jdxBest))
                    {
                        setup.equalVariables[val].main_symmetry = jdxBest;
                    }
                }
            }
            subsetsUsers.clear();
    ...
    for (int i = 0; i <= setup.numUser; i++)
    {
        ...
        {
            current.clear();
            generateSubsets(allFile, i, 0, current, subsetsFiles, setup.numFile + setup.numUser);
            for (auto filevec : subsetsFiles)
            {
                int idx = 0;
                for (auto ix : filevec)
                {
                ...
                current.clear();
                generateSubsets(allUser, j, 0, current, subsetsUsers, setup.numFile + setup.numUser);
                int jdxBest = 0;
                vector<int> cachevecAll;
                for (auto cachevec : subsetsUsers)
                {
                    int jdx = idx;
                    for (auto jx : cachevec)
                    {
    ...
    // For each subset of variables, add the constraint that the sum of the variables in the subset is equal to the sum of the variables in the subset for all permutations
    for (int subsetSize = 1; subsetSize < int(allPermutations.begin()->size()) - setup.commonInformations; subsetSize++)
    {
        vector<int> current;
        generateSubsets(allVariables, subsetSize, 0, current, subsets, allPermutations.begin()->size() + setup.commonInformations);
    }
    ...
    cout << "Number of subsets to process: " << subsets.size() << endl;
    // Loop over each subset in your collection of subsets.
    // #pragma omp parallel for schedule(dynamic, 1) // Use dynamic scheduling for better load balancing
    for (auto &subset : subsets)
    {
        ...
        for (; it != allPermutations.end(); ++it)
        {
            ...
            alreadyProcessed.insert(sumPermutation);
            totalEquations++;
            if (setup.equalVariables[sumPermutation].main_symmetry == -1)
            {
                if (setup.equalVariables[sumSubset].main_symmetry == -1)
                {
                    setup.equalVariables[sumPermutation].main_symmetry = sumSubset;
                }
                else
                {
                    setup.equalVariables[sumPermutation].main_symmetry = setup.equalVariables[sumSubset].main_symmetry;
                }
            }
            // TODO add  Gurobi constraint: sumSubset == sumPermutation.
            // #pragma omp critical
    ...
    cout << "Total symmetry equations: " << totalEquations << endl;
    int totalSymmetries = 0;
    int finalPosition = 0;
    for (size_t i = 0; i < setup.equalVariables.size(); i++)
    {
        if (setup.equalVariables[i].main_symmetry != -1)
        {
            int symmVal = setup.equalVariables[i].main_symmetry;
            if (setup.equalVariables[symmVal].main_symmetry != -1)
            {
                setup.equalVariables[i].main_symmetry = setup.equalVariables[symmVal].main_symmetry;
            }
        }
    }
    ...
    if(setup.commonInformations > 0)
    {
        for (int i=0; i<=setup.demands.size(); i++)
        {
            int demandPosition = i == 0 ? 0 : (1 << (setup.numUser + setup.numFile + setup.commonInformations + i - 1));
            for(int i=0; i<setup.commonInformations; i++)
            {
                int commonInfoPosition = 1 << (setup.numUser + setup.numFile + i);
                for(int j=demandPosition; j<demandPosition+commonInfoPosition; j++)
                {
                    if(setup.equalVariables[j].main_symmetry == -1)
                    {
                        setup.equalVariables[j + commonInfoPosition].main_symmetry = -1;
                    }
                    else
                    {
                        setup.equalVariables[j + commonInfoPosition].main_symmetry = setup.equalVariables[j].main_symmetry + commonInfoPosition;
                    }
                }
            }
        }
    }
    // Print all the main symmetries
    for (size_t i = 0; i < setup.equalVariables.size(); i++)
    {
        if (setup.equalVariables[i].main_symmetry == -1)
        {
            setup.equalVariables[i].final_position = finalPosition;
            finalPosition++;
        }
        if (Debug > 0)
        {
            outFile << "Variable #" << i << ": ";
            printOnTotalValueFile(i, setup);
            if (setup.equalVariables[i].main_symmetry != -1)
            {
                outFile << " is symmetric to Variable #";
                printOnTotalValueFile(setup.equalVariables[i].main_symmetry, setup);
            }
            else
            {
                outFile << " is unique and assigned final position " << setup.equalVariables[i].final_position;
            }
            outFile << endl;
        }
    ...
    cout << "Total symmetries found: " << totalSymmetries << endl;
    final_NumVar = int(setup.equalVariables.size()) - totalSymmetries + 1;
    cout << "Final number of variables after symmetry reduction: " << final_NumVar << endl;
    return final_NumVar;
}
...
int main(int argc, char *argv[])
{
    ...
    outFile = ofstream("output_delVar.txt");
    ...
    printSetup(setup);
    std::cout << "numSingleVar = " << numSingleVar << "\n";

    size_t vecSize = 1ULL << numSingleVar; // 512
    std::cout << "DEBUG: Before resize, resizing to " << vecSize << std::endl;

    setup.equalVariables.resize(vecSize);
    std::cout << "DEBUG: After resize\n";

    for (int i = 0; i < int(vecSize); i++)
    {
        setup.equalVariables[i].variable = i;
        setup.equalVariables[i].main_symmetry = -1;
        setup.equalVariables[i].final_position = -1;
        // cout << "Equal variable " << i << ": " << setup.equalVariables[i].variable << ", main_symmetry: " << setup.equalVariables[i].main_symmetry << endl;
    }

    cout << "Number of single variables: " << numSingleVar << endl;
    ...
    try
    {
        ...
        if (!setup.fullVariables)
        {
            ...
            SymVar = maxVar;
            setup.symmetryCheck.resize(SymVar, 0);
            setup.maxValueSubsets.resize(SymVar, 0);
        }
        else
        {
            SymVar = 1ULL << (setup.numUser + setup.numFile + setup.commonInformations + numDemands);
            setup.symmetryCheck.resize(SymVar, 0);
            setup.maxValueSubsets.resize(SymVar, 0);
            maxVar = findEqualVariablesUsingSymmetry(setup, numSingleVar);
            symDelete = true;
            bitmapTable = model.addVars(maxVar, GRB_CONTINUOUS);
        }

        renameVariables(bitmapTable, setup, numDemands, maxVar, model, symDelete);
        

        modelSetup(model);

        addModelConstraints(setup, model, bitmapTable, startChrono, maxVar, numSingleVar, SymVar);
